...
//	t_sphilo		phil[in->nph];
	...
	in->i = -1;
//	pthread_mutex_init (&in->spf, NULL);//inicio mutex de impresion
	while (++in->i < in->nph)
	{
...
//		free (&phil[in->j].in->mutex[in->j]);
//		free (&phil[in->j]);
	}
//	pthread_mutex_destroy (&in->spf);
	return (0);
}
...
void	*ft_create(void *phil_anterior)//funcion que retiene los hilos
{
	...
	while (phil->in->sd == 2)//bucle para retener los hilos 
	{
		if (phil->id % 2 == 0)//empiezan comiendo los impares (id+1 por el filosofo 0)
		{
			...
			if (ft_take (phil) == 1)
			{	
//				pthread_mutex_unlock (&phil->in->spf);
				return ((void *)1);
			}
			phil->sw = 1;
		}
		else
		{
			if (phil->sw == 0)
			{
				...
				usleep (50);//pequeño retardo para los filosofos pares y no se atropellen entre todos
//			printf ("id(%d) con time: %ld y sew: %d voy\n", phil->id + 1, ft_time(phil->in->tstart), phil->sem);
			if (ft_take (phil) == 1)
			{	
//				pthread_mutex_unlock (&phil->in->spf);
				return ((void *)1);
...
//		if (ft_usleep (phil->in->tthink, phil) == 1)//tthink es 0, se podria quitar
//			return (1);
		phil->se = 0;
		phil->ss = 0;
		phil->sp = 1;
